...
- [x] 新增五套角色主题：夏鸣星暖橙、左然酒红、沈星回星河紫、易遇咖啡、白起红黑。
- [x] 全站内联 SVG 图标系统（含导航、聊天、设置、通话浮层），不再混用几何符号与 emoji 图标
- [x] 消息操作改为长按触发（桌面端悬停浮层），去掉每条消息常驻的操作按钮，聊天区更清爽
- [x] 群聊头像只取前两位成员首字并约束溢出，避免小圆内文字溢出
- [x] 全局字体大小调节：一个滑杆 / 五档预设，底部导航、设置页、聊天气泡等全部文字同步缩放
- [x] 群聊头像支持从相册导入静态图片或短视频，也可一键恢复成员首字
- [x] 超长历史的分块摘要与分块记忆提取：几十万字存档不再因单次请求超上下文而永久卡住，
      按 token 预算一段一段推进，每段完成即落库，摘要面板显示剩余未压缩条数
...
- [x] 最近 15 条消息始终保留完整原文。
- [x] 更早消息通过 `chat.summary` 压缩，不再逐条重复发送。
- [x] 消息超过 30 条且新增未压缩消息达到 20 条时，后台滚动更新摘要。
- [x] 摘要保留关键事件、关系变化、承诺、用户偏好和事实，第三人称且控制在 200 字左右。
- [x] 长期记忆只从已被摘要覆盖的旧消息中提取，避免从未压缩原文重复提取。
- [x] 记忆提取与滚动摘要均为后台异步任务，失败只记录 `console.warn`。
- [x] 摘要和记忆调用分别累计 prompt 与 completion token 统计。
- [x] 聊天顶栏显示摘要覆盖进度和角色记忆数量。
